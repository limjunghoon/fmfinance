package com.fletamuto.common.control.fmgraph;

import android.graphics.Color;

public class Constants {
	
	//축 위치 상수 값
	public final static int BAR_AXIS_X_BOTTOM =0;
	public final static int BAR_AXIS_X_TOP =1;
	public final static int BAR_AXIS_X_CENTER =2;
	public final static int BAR_AXIS_Y_LEFT =3;
	public final static int BAR_AXIS_Y_RIGHT =4;
	public final static int BAR_AXIS_Y_CENTER =5;
		
	//눈금 모드
	public final static int BAR_GRADATION_MODE_NONE = 0;
	public final static int BAR_GRADATION_MODE_SHORT = 1;
	public final static int BAR_GRADATION_MODE_FULL = 2;
	
	//Default 값
	public final static int BAR_DEFAULT_PADDING = 20;
	public final static int BAR_DEFAULT_AXIS_THICKNESS = 2;
	public final static int BAR_DEFAULT_GRADATION_THICKNESS = 1;
	public final static int BAR_DEFAULT_GROUP_MEMBER_COUNT = 1;
	public final static int BAR_DEFAULT_BARGROUP_AND_BARGROUP_GAP = 30;
	public final static int BAR_DEFAULT_BAR_AND_BAR_GAP_IN_BAR_GROUP = 5;
	public final static int BAR_DEFAULT_BAR_WIDTH = 10;
	public final static int BAR_DEFAULT_GRADATION_COUNT = 5;
	
	public final static int PIE_DEFAULT_GRAPH_BOUNDARY_THICKNESS = 5;
	public final static int PIE_DEFAULT_GRAPH_PADDING = 20;

}
