...
//		}
	...
	public ArrayList<FinanceItem> getItemsFromCategoryID(int mainCategoryID, int year) {
		...
		queryBilder.setTables("liability, liability_main_category");
		queryBilder.appendWhere("liability.main_category=liability_main_category._id");
		Cursor c = queryBilder.query(db, null, "strftime('%Y', create_date)=? AND liability.main_category=?", params, null, null, null);
	...
	public Category getSubCategoryFromID(int categoryID) {
		...
		Cursor c = db.query("liability_sub_category", null, "_id=?", new String[]{String.valueOf(categoryID)}, null, null, null);
	...
	public ArrayList<Long> getTotalSubCategoryAmount(int subCategoryID, int year, int month, int beforMonthCount) {
		...
		for (int index = 0; index < beforMonthCount; index++) {
			...
			String query = "SELECT SUM(amount) FROM liability WHERE sub_category=? AND strftime('%Y-%m', create_date)=?";
			Cursor c = db.rawQuery(query, params);
	...
	public long getTotalMainCategoryAmount(int categoryID) {
		...
		String query = "SELECT SUM(amount) FROM liability WHERE main_category=?";
		Cursor c = db.rawQuery(query, params);
